...
const Header = () => {
  ...
  const handleLogout = async () => {
    ...
    if (response && success) {
      ...
      navigate("/auth/login"); // Navigate to login page after successful logout
      setShowUserMenu(false);
    } else if (error) {
            ...
            {showUserMenu && (
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-50 scale-in">
                ...
                <div className="py-2">
                  ...
                  >
                    {loading ? (
                      <span className="animate-spin w-4 h-4 border-4 border-t-transparent border-red-500 rounded-full mr-3"></span>
                    ) : (
